...
def _stub_catalog_default(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setattr("omnigent.inner.pi_settings.DEFAULT_PI_AGENT_DIR", tmp_path / "global-pi")
    monkeypatch.setattr(
...
def test_key_provider_model_options_degrade_to_default_when_listing_fails(
    ...
    assert {o["model"] for o in options} == {"zai/glm-5.3"}
